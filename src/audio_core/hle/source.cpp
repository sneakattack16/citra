...
void Source::MixInto(QuadFrame32& dest, size_t intermediate_mix_id) const {
    ...
    const std::array<float, 4>& gains = state.gains[intermediate_mix_id];
...
void Source::ParseConfig(SourceConfiguration::Configuration& config, const s16_le adpcm_coeffs[16]) {
    ...
    if (config.gain_0_dirty) {
        ...
        std::transform(config.gain[0], config.gain[0] + state.gains[0].size(), state.gains[0].begin(),
    ...
    if (config.gain_1_dirty) {
        ...
        std::transform(config.gain[1], config.gain[1] + state.gains[1].size(), state.gains[1].begin(),
            [](const auto& coeff) { return static_cast<float>(coeff); });
    ...
    if (config.gain_2_dirty) {
        ...
        std::transform(config.gain[2], config.gain[2] + state.gains[2].size(), state.gains[2].begin(),
            [](const auto& coeff) { return static_cast<float>(coeff); });
...
void Source::AdvanceFrame() {
    ...
    while (frame_position < current_frame.size()) {
        ...
        size_t size_to_copy = std::min(state.current_buffer.size(), current_frame.size() - frame_position);
...
bool Source::DequeueBuffer() {
    ASSERT(state.current_buffer.empty());
    ...
    LOG_TRACE(Audio_DSP, "Source::DequeueBuffer(): source_id=%u buffer_id=%u from_queue=%d current_buffer.size()=%zu",
